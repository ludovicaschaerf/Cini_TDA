...
def main(data_dir='/scratch/students/schaerf/', replica_dir='/mnt/project_replica/datasets/cini/', batch_size=8, num_epochs=1, model_name='resnext-101', device='cuda', resolution=480, num_c=10):
    ...
    model = train_replica(model, dts, dataset_sizes, uid2path, device=device, data_dir=data_dir, replica_dir=replica_dir, num_epochs=num_epochs, num_c=num_c, model_name=model_name, resolution=resolution, batch_size=batch_size)
    torch.save(model.state_dict(), data_dir + "models/model_weights_" + now + model_name)


# whats up with attention https://github.com/SeguinBe/Replica-Search/blob/master/replica_learn/layers.py 
...
if __name__ == "__main__":
    ...
    main(args.data_dir, args.replica_dir, args.batch_size, args.num_epochs, args.model_name, args.device, args.resolution, args.num_c)