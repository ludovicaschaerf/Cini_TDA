...
def get_embedding(img, model, type="fine_tune", device="cpu"):
    """Get numpy embedding using model.

    Args:
        img (tensor): input image
        model: model 
        type (str, optional): Whether used during fine tuning. Defaults to "fine_tune".
    ...
    """
    if type == "fine_tune":
        embedding = model.predict(img.squeeze(1).to(device))[
            0].cpu().detach().numpy()
    else:
...
def get_embedding_test(path, model, device, resolution):
    """Get numpy embedding using model without augmentation.
...
def make_training_set_orig(embeddings, train_test, data, data_dir, uid2path, epoch=10, n=10):
    """Creates the training information.

    Args:
        embeddings (array): embeddings of all the images
        train_test (pd.DataFrame): information on annotated images
        data (pd.DataFrame): information on all images
        data_dir (Path or str): directory where images are stored
        uid2path (dict): dictionary from unique identifier to path
        epoch (int, optional): epoch this is being called at. Defaults to 10.
        n (int, optional): number of Cs to sample. Defaults to 10.

    Returns:
        pd.DataFrame: training information dataset
    """
    tree = make_tree_orig(embeddings)
    Cs = []
    for i in tqdm(range(train_test.shape[0])):
        list_theo = (
            list(train_test[train_test["img1"] ==
                 train_test["uid"][i]]["img2"])
            + list(train_test[train_test["img2"] ==
                   train_test["uid"][i]]["img1"])
            + [train_test["uid"][i]]
        )
    ...
    final = final[final['C_path'].notnull() & final['A_path'].notnull(
    ) & final['B_path'].notnull()]  # .sample(frac=0.5)
    print(final.shape)
    print(final.tail())

    print(epoch)
    final[final['set'] == 'train'].reset_index().to_csv(
        data_dir + 'dataset/abc_train_' + str(epoch) + '.csv')
    final[final['set'] == 'test'].reset_index().to_csv(
        data_dir + 'dataset/abc_test_' + str(epoch) + '.csv')
    final[final['set'] == 'val'].sample(frac=0.1).reset_index().to_csv(
...
def make_tree_orig(embeds, reverse_map=False):
    """Creates BallTree to query embeddings.

    Args:
        embeds (np.array): embeddings of all images
        reverse_map (bool, optional): map from index to embedding. Defaults to False.
...
def find_most_similar_orig(uid, tree, embeds, uids, n=401, similarities=False):
    """Query inside the tree

    Args:
        uid (str): UID of image to query
        tree (tree): BallTree
        embeds (np.array): embeddings of all images
        uids (list): list of unique identifiers to use as reverse map
        n (int, optional): number of results. Defaults to 401.
        similarities (bool, optional): whether to also return the similarity. Defaults to False.

    Returns:
        list: UIDs of most similar images
    """
...
def find_most_similar_no_theo(uid, tree, embeds, uids, list_theo, n=401):
    """Query inside the tree and return results not in B.
    ...
    Args:
        ...
        embeds (np.array): embeddings of all images
        uids (list): list of unique identifiers to use as reverse map
        list_theo (list): list of Bs
        n (int, optional): number of results. Defaults to 401.
        
    Returns:
        list: UIDs of most similar images excluding Bs.
    """
    img = np.vstack(embeds[embeds[:, 0] == uid][:, 1]).reshape(1, -1)
    cv = tree.query(img, k=n)[1][0]
    # not in uids_match
    return [uids[c] for c in cv if uids[c] not in list_theo]
...
def find_pos_matches(uids_sim, uids_match, how="all", n=400):
    matched = list(
        filter(lambda i: uids_sim[i] in uids_match, range(len(uids_sim))))
...
def get_train_test_split(metadata, morphograph):
    ...
    positives.columns = ["uid_connection", "img1",
                         "img2", "type", "annotated", "cluster_file"]
    ...
    positives = pd.concat(
        [
            metadata.merge(positives, left_on="uid",
                           right_on="img1", how="inner"),
            metadata.merge(positives, left_on="uid",
                           right_on="img2", how="inner"),
        ],
        axis=0,
    ).reset_index()

    # adding set specification to df
    mapper = {it: number for number,
              nodes in enumerate(components) for it in nodes}
...
def remove_duplicates(metadata, morphograph):
    ...
    positives = pd.concat(
        [
            metadata.merge(positives, left_on="uid",
                           right_on="img1", how="outer"),
            metadata.merge(positives, left_on="uid",
                           right_on="img2", how="outer"),
        ],
        axis=0,
    ).reset_index()
...
def show_images(img_names):
    ...
    Args:
        img_names (tuple): file names of the images A,B,C
    """
...
def preprocess_image(img_name, resolution=480):
    """Load and augment image before training.

    Args:
        img_name (str): path to image
        resolution (int, optional): resolution to load the image. Defaults to 480.
    ...
    img = img.convert('RGB')
    tfms = transforms.Compose(  # augmentation: random crop, color jitter, random flip and rotation
        [
            transforms.ToTensor(),
...
def preprocess_image_test(img_name, resolution=480):
    """Load image for testing. No augmentation.
...
def get_scores(embeddings, train_test, data, list_downloaded=False, reverse_map=False, set_split=['test', 'val']):
    ...
    for i in tqdm(range(train_test.shape[0])):
        if (train_test["img1"][i] in list_downloaded) and (train_test["img2"][i] in list_downloaded
            ...
            list_sim = find_most_similar_orig(
                train_test["uid"][i], tree, embeddings, reverse_map, n=min(
                    data.shape[0], 4000)
            )
    ...
    recall_400 = np.mean([recall_at_k(ranks[i], 400)
                         for i in range(len(ranks))])
    recall_200 = np.mean([recall_at_k(ranks[i], 200)
                         for i in range(len(ranks))])
    recall_100 = np.mean([recall_at_k(ranks[i], 100)
                         for i in range(len(ranks))])
...
def get_drawer(row_2, path, info=False):
    if type(row_2) != str and 'ImageURL' in row_2.columns:
        ...
        drawer = '/'.join(path.split('/')[4:]).split('.')[0]
...
def show_similars(row, embeddings, train_test, tree, reverse_map, uid2path, data):
    ...
    if row["set"].values[0] in ['train', 'val', 'test']:
        list_theo = (
            list(train_test[train_test["img1"] ==
                 row["uid"].values[0]]["img2"])
            + list(train_test[train_test["img2"] ==
                   row["uid"].values[0]]["img1"])
            + [row["uid"].values[0]]
        )
    else:
    ...
    for i in range(len(sim[:11])):
        ...
        axarr[i+1].set_title(info_2 + '\n' + 'Similarity: ' + str(
            np.round((1 - similarities[i]), 2)) + ', Grountruth: ' + correct)
        image = requests.get(get_drawer(row_2, catch(sim[i], uid2path)))

        axarr[i+1].imshow(Image.open(BytesIO(image.content)))  # replica_dir +

    plt.savefig('/scratch/students/schaerf/figures/' +
                row["uid"].values[0] + '.jpg')
    plt.show()

    sim_rerank, similarities_rerank = rerank_spatial(
        row["uid"].values[0], sim, uid2path, similarities=True)
    print('positions of matches after reranking',
          find_pos_matches(sim_rerank, list_theo, n=50))
    ...
    for i in range(len(sim_rerank[:11])):
        ...
        axarr[i+1].set_title(info_2 + '(rerank)\n' + 'Matches: ' +
                             str(similarities_rerank[i]) + ', Grountruth: ' + correct)
        drawer = catch(sim_rerank[i], uid2path).split('/')[0]
        img = catch(sim_rerank[i], uid2path).split('_')[1].split('.')[0]
        image = requests.get(
            f'https://dhlabsrv4.epfl.ch/iiif_replica/cini%2F{drawer}%2F{drawer}_{img}.jpg/full/300,/0/default.jpg')

        axarr[i+1].imshow(Image.open(BytesIO(image.content)))  # replica_dir +

    plt.savefig('/scratch/students/schaerf/figures/' +
                row["uid"].values[0] + '_rerank.jpg')
    plt.show()
    ...
    for i in range(len(list_theo)):
        ...
        if type(corresp) == str:
            ...
            # replica_dir +
            axarr[i].imshow(Image.open(BytesIO(image.content)))
...
def show_results(row, embeddings, train_test, data, uid2path, fig_dir):
    ...
    for i in range(len(sim[:11])):
        ...
        info_2 = str(row_2["Author"].values[0]) + '\n ' + \
            str(row_2["Description"].values[0])
        info = info_2 + '\n' + 'Similarity: ' + \
            str(np.round((1 - similarities[i]), 2)
                ) + ', Grountruth: ' + correct
        axarr[i+1].set_title("\n".join(wrap(info, 25)))
        image = requests.get(get_drawer(row_2, catch(sim[i], uid2path)))
        ...
        try:
            ...
            # replica_dir +
            axarr[i+1].imshow(Image.open(BytesIO(image.content)))
    ...
    plt.tight_layout()
    plt.savefig(fig_dir + 'retrieval_' + row["Author"].values[0].split()[
                0] + '_' + row["Description"].values[0].split()[0] + '.jpg')
    plt.show()
...
def show_clusters(cluster_num, morpho, uid2path, fig_dir, title):
    ...
    f, axarr = plt.subplots(
        cluster.shape[0] // 4 + 1, 4, figsize=(12, (cluster.shape[0] // 4 + 1) * 5))
    axarr = axarr.flatten()
    ...
    for i, row in cluster.reset_index().iterrows():
        ...
        image = requests.get(get_drawer(
            pd.DataFrame(row), catch(row['uid'], uid2path)))
    ...
    if cluster.shape[0] > 0:
        plt.subplots_adjust(wspace=0, hspace=0)
        plt.tight_layout()
        plt.savefig(fig_dir + str(cluster_num) + '_' +
                    row["Author"].split()[0] + '_' + row["Description"].split()[0] + '.jpg')
        plt.show()
...
def show_suggestions(row, embeddings, train_test, tree, reverse_map, uid2path, data):
    ...
    for i in range(len(sim)):
        row_2 = data[data['uid'] == sim[i]]
        info_2 = row_2["AuthorOriginal"].values[0] + \
            ' ' + row_2["Description"].values[0]
        axarr[i+1].set_title(info_2)
