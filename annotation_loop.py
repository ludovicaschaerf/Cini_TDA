...
def get_data(data_dir="./data/", size=1000):
    data = (
        pd.read_csv(data_dir + "dedup_data.csv")
        .drop(columns=["Unnamed: 0", "level_0"])
        .sample(size)
    )  #'full_data.csv').drop(columns=['Unnamed: 0', 'level_0'])
    path = "./data/"
...
def main(
    ...
    if not embds:
        embeddings, data, tree, reverse_map = get_data(
            data_dir, size
        )  # np.load(data_dir + 'embeddings/benoit.npy',allow_pickle=True)
    ...
    similars = input(
        "which ones are VERY similar but NOT equal in pose? \n Write the numbers separated by commas"
    )

    uids_sim = [sim[int(i.strip())] for i in similars.split(",") if i != ""]

    morpho_graph = morpho_complete.loc[
        ((morpho_complete.img1.isin(uids)) & (morpho_complete.img2.isin(uids)))
    ][["uid", "img1", "img2", "type", "annotated"]]
    new_morphs = pd.DataFrame(
        [
            [a[:16] + uids_sim[i][16:], a, uids_sim[i], "POSITIVE", now]
            for i in range(len(uids_sim))
        ],
        columns=["uid", "img1", "img2", "type", "annotated"],
    )
