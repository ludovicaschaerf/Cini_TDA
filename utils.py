...
def make_training_set(data_dir, model, subset, device="cpu"):
    embeddings = [
        get_embedding(preprocess_image(data_dir + uid + ".jpg"), model, device=device)
        .squeeze(3)
        .squeeze(1)
        .squeeze(0)
        for uid in tqdm(subset["uid"].unique())
    ]
    ...
    for i in tqdm(range(train_test.shape[0])):
        ...
        list_ = find_most_similar_list(
            train_test["uid"][i],
            tree,
            embeddings_new,
            list(subset["uid"].unique()),
            list_theo,
        )
        Cs.append(list_)
